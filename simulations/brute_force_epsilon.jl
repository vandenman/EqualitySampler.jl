...
mutable struct WarningCountingLogger <: AbstractLogger
	count::Int
...
Logging.shouldlog(::WarningCountingLogger, args...) = Logging.shouldlog(global_logger(), args...)
Logging.min_enabled_level(::WarningCountingLogger) = Logging.min_enabled_level(global_logger())
...
function Logging.handle_message(logger::WarningCountingLogger, level, message, _module, group, id, filepath, line; kwargs...)
	if level == Logging.Warn && group == :hamiltonian && endswith(filepath, "hamiltonian.jl")#, 47
		...
		Logging.handle_message(global_logger(), level, message, _module, group, id, filepath, line; kwargs...)
	end
end
