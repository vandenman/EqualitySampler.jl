...
function fit_lm(df::SimpleDataSet)
	...
	fit = GLM.lm(design_matrix, df.y)::GLM.LinearModel{GLM.LmResp{Vector{Float64}}, GLM.DensePredChol{Float64, LA.CholeskyPivoted{Float64, Matrix{Float64}}}}
	GLM.coef(fit), GLM.confint(fit), fit
end

function fit_lm(df, formula = SM.@formula(y ~ 1 + g))

	# TODO: don't use a dataframe but use the X, y directly?
	fit = GLM.lm(formula, df)::SM.TableRegressionModel{GLM.LinearModel{GLM.LmResp{Vector{Float64}}, GLM.DensePredChol{Float64, LA.CholeskyPivoted{Float64, Matrix{Float64}}}}, Matrix{Float64}}
...
function get_suff_stats(df::DF.DataFrame)
	...
	_, obs_mean, obs_var, obs_n = eachcol(
		DF.combine(DF.groupby(df, :g), :y => mean, :y => var, :y => length)
	)
...
@model function one_way_anova_mv_ss_submodel(obs_mean, obs_var, obs_n, Q, partition = nothing, ::Type{T} = Float64) where {T}
	...
	θ_r ~ MvNormal(LA.Diagonal(FillArrays.Fill(1.0, n_groups - 1)))
